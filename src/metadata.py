...
#note: als je java hebt geinstalleerd kun je de configure.bat file verwijderen uit het cmd_command

def make_logger(name: str, log_folder_path: str | Path):
    logger = logging.getLogger(name)
    log_folder_path = Path(log_folder_path)
    if not logger.handlers:
        handler = logging.FileHandler(filename = log_folder_path / f"{name}.log")
