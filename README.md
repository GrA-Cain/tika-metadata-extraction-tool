...
- [Tika-metadata-tool](#Tika-metadata-tool)
   ...
   - [Feature suggesties](#Features-suggesties)
...
* Een steekproef nemen van de data via de sample fractie instelling
...
## Feature suggesties en known issues
* Om .metadata.json files te genereren moet je de code in duiken (metadatapipeline class variable dictionary's) om de output te veranderen, dit moet flexibel in de UI kunnen
* "Group" DataFrames zijn hardcoded in de DataFramePipeline, alhoewel het relatief makkelijk is om nieuwe groups toe te voegen moet je hier ook de code in duiken. Daarbij worden "groups" en "namespace" als synoniem gebruikt, dit moet uniform. Er moet dus een feature ontwikkeld worden zodat je makkelijk nieuwe groups kan toevoegen en groups kan verwijderen.
* Alleen getest op een testset van ~200 files (.docx, .pdf, .msg, .jpg en .xlsx). Sommige functionaliteit (zoals duplicaten analyse) moet wellicht geoptimaliseerd worden als je met grote datasets werkt. 
